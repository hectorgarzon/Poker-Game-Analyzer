...
layout = html.Div(
    ...
    children=[
        ...
        html.Hr(),
        html.Div(id="dummy-output-players", style={"display": "none"}),
...
def _build_player_table(df: pd.DataFrame) -> Any:
        ...
        rows.append(
            {
                ...
                "hero_vs_player_net": round(float(row.get("hero_vs_player_net", 0)), 1),
    ...
    return dash_table.DataTable(
        ...
        columns=[
            ...
            {"name": "My Profit vs Him", "id": "hero_vs_player_net"},
...
def get_hero_hands_with_actions_existing_queries(db_path: str, hero_id: int) -> pd.DataFrame:
    ...
    Args:
        ...
        hero_id: ID del jugador héroe

    Returns:
        DataFrame con columnas: username, id, hand_id, net_profit,
        preflop_actions, flop_actions, turn_actions, river_actions
    """
    conn = get_connection(db_path)

    try:
        # 1. Obtener todas las manos del héroe usando la consulta existente
        hero_hands = get_hero_hand_players(conn, hero_id)

        # Obtener el nombre de usuario del héroe
        hero_info = conn.execute("SELECT id, username FROM players WHERE id = ?", (hero_id,)).fetchone()
        username = hero_info[1]

        # 2. Para cada mano, obtener las acciones usando la consulta existente
        results = []
        for _, row in hero_hands.iterrows():
            ...
            # Filtrar solo las acciones del héroe
            hero_actions = actions_df[actions_df['player_id'] == hero_id]

            # Procesar acciones por calle
...
def analyze_losing_action_combinations(df: pd.DataFrame) -> None:
    ...
    # 1. Filtrar manos donde el héroe vio al menos el flop
    ...
    print(f"Total perdido en estas combinaciones: ${worst_combos['total_loss'].sum():.2f}")
    print(f"Porcentaje de pérdidas totales: {worst_combos['total_loss'].sum() / df_filtered['net_profit'].sum() * 100:.2f}%")

@callback(
    Output("dummy-output-players", "children"),
    Input("_pages_location", "pathname"),
    prevent_initial_call=False  # Cambiado a False para que se ejecute siempre
)
def analyze_player_losses_on_load(pathname: str) -> None:
    """Analiza las combinaciones de acciones que más pérdidas generan al cargar la página."""
    print("\n🔍 Iniciando análisis de patrones de pérdida...")  # Mensaje de depuración

    if pathname != "/players":
        print("No es la página de players, cancelando análisis")
        raise dash.exceptions.PreventUpdate

    # Obtener el ID del héroe
    db_path = _get_db_path()
    print(f"Ruta de la base de datos: {db_path}")  # Mensaje de depuración

    hero_id = _get_hero_player_id(db_path)
    if not hero_id:
        print("⚠️ No se pudo obtener el ID del héroe")
        return None

    print(f"ID del héroe: {hero_id}")  # Mensaje de depuración
    ...
    if df.empty:
        print("⚠️ No se encontraron manos para analizar")
        return None

    print(f"Se encontraron {len(df)} manos para analizar")  # Mensaje de depuración

    # Analizar las combinaciones de acciones
    print("Analizando combinaciones de acciones...")
    analyze_losing_action_combinations(df)
