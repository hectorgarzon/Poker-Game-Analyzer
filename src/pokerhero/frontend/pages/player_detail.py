...
def layout(player_id: str = None, **kwargs):
    ...
    # Obtener las manos y resultados del jugador
    hands_query = """
        SELECT h.source_hand_id as hand_id, hp.net_result
        FROM hand_players hp
        JOIN hands h ON hp.hand_id = h.id
        WHERE hp.player_id = ?
    """
    df_hands = pd.read_sql_query(hands_query, conn, params=(player_id,))
    conn.close()
